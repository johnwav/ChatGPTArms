...
export class WeatherArm {
  ...
  public async amINeeded(convo) {
    ...
    if (
      messages.length > 1 &&
      messages[messages.length - 2].hasOwnProperty("need_zip")
    ) {
      return 2;
    }
    const triggerTerms = [
    ...
    if (await arms.is_cos_sim_close(convo.chatQuery, triggerTerms, 0.85)) {
      return 1;
    }
  ...
  public async checkWeatherOutside(convo) {
    ...
    if (typeof zipCode !== "string") {
      let content = "Please enter a US zip code.";
      return {
        content: content,
        role: "assistant",
      };
    ...
    let content =
      "The current temperature out is " +
